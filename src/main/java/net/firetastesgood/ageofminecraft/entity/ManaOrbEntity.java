...
public class ManaOrbEntity extends Entity {
    ...
    // --- synced value so client renderers can read it ---
    ...
    public ManaOrbEntity(Level level, double x, double y, double z, int value) {
        ...
        this.setPos(x, y, z);
        this.setValue(value); // important: sync to client
    }
    ...
    // --- lifecycle / behavior (server-authoritative physics) ---
    @Override
    public void tick() {
        super.tick();
    ...
    private void mergeNearby(double radius) {
        // Find the single best candidate (closest), ignore very young orbs
